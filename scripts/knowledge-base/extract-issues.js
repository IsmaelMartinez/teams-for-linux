...
class GitHubIssuesExtractor {
  ...
  async extractIssues(isIncremental = false) {
    ...
    while (hasMorePages) {
      try {
        ...
        const issues = await this.callGitHubAPI(params);
        ...
        if (page % CONFIG.PROGRESS_INTERVAL === 0) {
          console.log(
            `📊 Progress: ${this.extractedIssues.length} issues extracted from ${page} pages`
          );
        }
        ...
        if (consecutiveErrors >= CONFIG.MAX_RETRIES) {
          ...
          throw new Error(
            `Failed after ${CONFIG.MAX_RETRIES} consecutive errors`
          );
        }
  ...
  async processIssues(issues) {
    ...
    for (const issue of issues) {
      try {
        ...
        console.warn(
          `⚠️ Failed to process issue #${issue.number}:`,
          error.message
        );
  ...
  shouldExtractComments(issue) {
    ...
    return issue.state === "closed" || issue.comments >= 3 || hasRelevantLabel;
  }
  ...
  async extractComments(issueNumber) {
    try {
      ...
      if (typeof mcp_github_get_issue_comments !== 'undefined') {
        ...
        console.log(`🔧 MCP tools not available, skipping comment extraction`);
        return []; // Return empty array in simulation mode
      }
    } catch (error) {
      console.warn(
        `⚠️ Failed to extract comments for issue #${issueNumber}:`,
        error.message
      );
      return [];
    }
  ...
  validateData() {
    ...
    const validation = {
      ...
      issuesWithComments: this.extractedIssues.filter(
        (issue) => issue.comments?.length > 0
      ).length,
      closedIssues: this.extractedIssues.filter(
        (issue) => issue.state === "closed"
      ).length,
      openIssues: this.extractedIssues.filter((issue) => issue.state === "open")
        .length,
      pullRequests: this.extractedIssues.filter(
        (issue) => issue.is_pull_request
      ).length,
    ...
    if (validation.totalIssues !== validation.uniqueIssues) {
      console.warn(
        `⚠️ Found ${
          validation.totalIssues - validation.uniqueIssues
        } duplicate issues`
      );
    }
  ...
  /**
   * Call GitHub API using MCP tools or fallback to simulation
   */
  async callGitHubAPI(params) {
    ...
    if (typeof mcp_github_list_issues !== 'undefined') {
      ...
      console.log(`🔧 MCP tools not available, using simulation mode`);
      return this.simulateApiCall(params);
    }
  }
    ...
    return [
      {
        ...
        html_url:
          "https://github.com/IsmaelMartinez/teams-for-linux/issues/1773",
      },
    ];
  }
