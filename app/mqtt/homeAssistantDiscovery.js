...
{ component: 'binary_sensor', objectId: 'in_call', config: this.#buildBinarySensorConfig('Teams In Call', 'in_call', 'in-call', 'mdi:phone-in-talk') },
{ component: 'button', objectId: 'toggle_mute', config: this.#buildButtonConfig('Teams Toggle Mute', 'toggle_mute', 'toggle-mute', 'mdi:microphone-off') },
{ component: 'button', objectId: 'toggle_video', config: this.#buildButtonConfig('Teams Toggle Video', 'toggle_video', 'toggle-video', 'mdi:camera-off') },
{ component: 'button', objectId: 'toggle_hand_raise', config: this.#buildButtonConfig('Teams Toggle Hand Raise', 'toggle_hand_raise', 'toggle-hand-raise', 'mdi:hand-back-left') },
];
...
/**
 * Remove all HA discovery entities by publishing empty retained payloads.
 * Only call when intentionally disabling the feature; for normal disconnects
 * the availability_topic handles the unavailable state automatically.
 */
async clearDiscovery() {
const entities = [
{ component: 'sensor', objectId: 'status' },
{ component: 'binary_sensor', objectId: 'in_call' },
{ component: 'button', objectId: 'toggle_mute' },
{ component: 'button', objectId: 'toggle_video' },
{ component: 'button', objectId: 'toggle_hand_raise' },
];

for (const { component, objectId } of entities) {
const topic = `${this.#discoveryPrefix}/${component}/${this.#deviceId}/${objectId}/config`;
await this.#mqttClient.publish(topic, '', { retain: true });
}

console.info('[HomeAssistantDiscovery] Cleared discovery configurations');
}
}
