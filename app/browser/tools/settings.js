...
async function retrieve(event) {
  ...
  if (!clientPreferences) {
    ...
    const settings = {
      ...
      devices:
        inst.controller.callingService._deviceManagerService.deviceManager.getSelectedDevices(),
    };
    settings.devices.camera = getDeviceLabelFromId(
      inst.controller,
      settings.devices.camera,
      1,
    );
    settings.devices.microphone = getDeviceLabelFromId(
      inst.controller,
      settings.devices.microphone,
      2,
    );
    settings.devices.speaker = getDeviceLabelFromId(
      inst.controller,
...
function getDeviceLabelFromId(controller, id, kind) {
  const item = controller.callingService._deviceManagerService.devices.filter(
    (f) => f.id === id && f.kind === kind,
  )[0];
...
async function restore(event, ...args) {
  ...
  if (!clientPreferences) {
    ...
    args[0].devices.camera = getDeviceIdFromLabel(
      inst.controller,
      args[0].devices.camera,
      1,
    );
    args[0].devices.microphone = getDeviceIdFromLabel(
      inst.controller,
      args[0].devices.microphone,
      2,
    );
    args[0].devices.speaker = getDeviceIdFromLabel(
      inst.controller,
      args[0].devices.speaker,
      3,
    );
    inst.controller.callingService._deviceManagerService.deviceManager.selectDevices(
      args[0].devices,
    );
