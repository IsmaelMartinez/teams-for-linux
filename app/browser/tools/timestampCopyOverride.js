...
class TimestampCopyOverride {
    ...
    applyOverride() {
        ...
        if (!coreServices || !coreServices.coreSettings) return;

        const coreSettings = coreServices.coreSettings;

        if (coreSettings.get('compose').disableTimestampOnCopy === this.config.coreSettingsComposeDisableTimestampOnCopy) {
            console.log('Setting coreSettingsComposeDisableTimestampOnCopy is correct, stopping polling');
            this.stop();
        }

        const overrides = {
            disableTimestampOnCopy: this.config.coreSettingsComposeDisableTimestampOnCopy
        }
