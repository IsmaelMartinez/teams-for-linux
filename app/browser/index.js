(function () {
  ...
  class CustomNotification {
    constructor(title, options) {
      ...
      console.debug("CustomNotification created:", {
        title,
        options,
        notificationMethod: config.notificationMethod,
      });

      // Potential issue: requireInteraction=false makes notifications auto-dismiss quickly
      // Let's try setting it to true for better visibility
      options.requireInteraction =
        config.notificationRequireInteraction !== false; // Default to true unless explicitly disabled
      ...
      if (config.notificationMethod === "web") {
        ...
        // DEBUG: Log when native notification is created
        const notification = new classicNotification(title, options);
        console.debug("Native browser notification created:", notification);

        // Add event listeners to track notification lifecycle
        notification.onshow = () => console.debug("Notification shown:", title);
        notification.onclose = () =>
          console.debug("Notification closed:", title);
        notification.onerror = (error) =>
          console.debug("Notification error:", title, error);
        notification.onclick = () =>
          console.debug("Notification clicked:", title);
