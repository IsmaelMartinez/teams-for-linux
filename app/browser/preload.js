...
document.addEventListener('DOMContentLoaded', async () => {
	...
	try {
		...
		if (config.useMutationTitleLogic) {
			const mutationTitle = await import("./tools/mutationTitle.js");
			mutationTitle.default.init(config);
		}
		...
		const modules = [
			{ name: "zoom", path: "./tools/zoom.js" },
			{ name: "shortcuts", path: "./tools/shortcuts.js" },
			{ name: "settings", path: "./tools/settings.js" },
			{ name: "theme", path: "./tools/theme.js" },
			{ name: "emulatePlatform", path: "./tools/emulatePlatform.js" },
			{ name: "timestampCopyOverride", path: "./tools/timestampCopyOverride.js" },
			{ name: "trayIconRenderer", path: "./tools/trayIconRenderer.js" },
			{ name: "mqttStatusMonitor", path: "./tools/mqttStatusMonitor.js" },
			{ name: "disableAutogain", path: "./tools/disableAutogain.js" },
			{ name: "navigationButtons", path: "./tools/navigationButtons.js" },
			{ name: "framelessTweaks", path: "./tools/frameless.js" }
		];
		...
		for (const module of modules) {
			try {
				...
				if (module.name === "settings" || module.name === "theme" || module.name === "trayIconRenderer" || module.name === "mqttStatusMonitor") {
					moduleInstance.default.init(config, ipcRenderer);
				} else {
					moduleInstance.default.init(config);
				}
				successCount++;
			} catch (err) {
		...
		try {
			const ActivityManagerModule = await import("./notifications/activityManager.js");
			const ActivityManager = ActivityManagerModule.default;
