	...
	type: "object",
      ...
      auth: {
        ...
        fields: {
          ...
          "webLogin.totpCommand": {
            type: "string",
            describe:
              "Command whose first stdout line is pre-filled into the web sign-in one-time-code field on the authenticator-app (TOTP) page. Runs in a shell (e.g. 'pass otp teams'); whitespace is stripped so grouped output like '123 456' works. Empty disables it. Runs only once the code field is on screen, so the code is fetched fresh. The app stores no secret. Microsoft serves SMS, voice and email codes through the same field, so this is ignored (with a warning) when auth.webLogin.verifyMethod selects one of those methods.",
          },
          "webLogin.totpSelector": {
            type: "string",
            describe:
              "Extra CSS selector for the one-time-code input, for identity providers the built-in selectors miss (issue #2869). Added to the built-in list rather than replacing it, so Microsoft pages keep working; e.g. 'input[name=\"credentials.totp\"]' for Okta. An invalid selector is ignored and only the built-ins are used. Only has an effect with auth.webLogin.totpCommand set.",
          },
          "webLogin.extraHosts": {
            type: "array",
            describe:
