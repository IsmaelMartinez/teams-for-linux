...
| `auth.webLogin.totpSelector` | `string` | `""` | Extra CSS selector for the one-time-code field, for identity providers the built-in selectors miss (e.g. `input[name="credentials.totp"]` for Okta). Added to the built-in list rather than replacing it. Only has an effect with `auth.webLogin.totpCommand` set. |
...
The code field is matched by the selectors Microsoft and Okta use. If your identity provider renders something else, add its selector with `auth.webLogin.totpSelector` and check the `[SSO_PREFILL] Login page handled` log line: `otcField: false` means nothing matched.
