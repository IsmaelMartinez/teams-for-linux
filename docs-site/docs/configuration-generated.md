...
| `mqtt.meetingStartDetection.enabled` | `boolean` | `false` | Enable best-effort detection of the 'started the meeting' toast, published as a pulse to the meeting-started topic (experimental, see issue #2587). |
| `mqtt.meetingStartDetection.patterns` | `array` | `["started the meeting"]` | Case-insensitive regular expressions matched against toast text. The default only covers English Teams; override for other UI languages or if Microsoft rewords the toast. |
