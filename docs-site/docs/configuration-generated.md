...
| `auth` | `object` | `{"intune":{"enabled":false,"user":""},"webauthn":{"enabled":false,"debug":false}}` | Authentication configuration. auth.webauthn.enabled turns on hardware security key support on Linux (requires fido2-tools). auth.webauthn.debug enables verbose diagnostic logs, intended for beta testers only. |
