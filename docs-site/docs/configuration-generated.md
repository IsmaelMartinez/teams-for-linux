...
| `notifications.trayBadgeEnabled` | `boolean` | `true` | Draw the unread count badge on the tray icon. |
| `notifications.taskbarBadgeEnabled` | `boolean` | `false` | Show the unread count badge on the taskbar/dock icon (macOS dock, and Linux docks implementing the Unity LauncherEntry protocol). Off by default while the feature is in early development. |
