...
- **Duplicate notifications.** The `window.Notification` override in `preload.js` runs outside the module list; if the role does not gate it, the secondary window raises the same notifications as the main one. The role gates the override, and the e2e case asserts that a notification fired in the secondary window never reaches the main-process pipeline.
- **Double ring on incoming calls.** `ActivityManager` forwards `incoming-call-created` from both windows, so `browserWindowManager` would show the toast twice. The main process dedupes by ringing state: one toast per incoming call, whichever window reported it first, answered wherever the user clicks.
