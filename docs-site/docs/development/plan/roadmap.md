...
- **Config Schema as Single Source of Truth** ([#2597](https://github.com/IsmaelMartinez/teams-for-linux/issues/2597)): phases 0-2 shipped in v2.12.0 (drift fixes, generated reference + `config-schema.json` with CI drift guard, docs config explorer); 3a (`applyMode` + nested-field metadata, hard-failing generator lint) and 4 (warn-only startup validation) landed; 3b (in-app settings window) is the remaining phase ([research](../research/documentation-and-config-ux-research.md)). 3b is sequenced migration-first, so the leftover flat config options are cleaned up before the risky config-write path is built:
  1. Finalize the flat-to-nested mapping for the remaining flat options and add structured `renamedTo` metadata to them (naming owned by [configuration organization research](../research/configuration-organization-research.md)).
  ...
  3. Build the restart-only settings MVP, reusing the `legacyConfigStore` override-on-boot layering and allowlisting writes to schema keys with type/choice plus deep prototype-pollution validation; defer the live-apply subset and the object-typed forms.
