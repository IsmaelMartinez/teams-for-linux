...
- **SSO Web Sign-in Pre-fill** ([#2794](https://github.com/IsmaelMartinez/teams-for-linux/issues/2794), merged in [#2761](https://github.com/IsmaelMartinez/teams-for-linux/pull/2761), opt-in via `auth.webLogin.*`): account and password pre-fill on the Microsoft/federated web login form, with optional auto-advance and MFA method selection. Authenticator one-time codes follow in [#2874](https://github.com/IsmaelMartinez/teams-for-linux/pull/2874) ([#2869](https://github.com/IsmaelMartinez/teams-for-linux/issues/2869)), which also covers non-Microsoft code fields via `auth.webLogin.totpSelector`. Passkey-aware flows only if requested.
