...
>
> **Last Generated**: 2026-05-26T14:32:52.086Z
...
| `window-error` | Event | Log renderer-side uncaught window errors | [`app/index.js:335`](https://github.com/IsmaelMartinez/teams-for-linux/blob/develop/app/index.js#L335) |
...
| Channel | Type | Description | Location |
|---------|------|-------------|----------|
| `delete-sticker` | Request/Response | Delete a sticker file from the sticker folder. Validates the requested name/subfolder do not contain path-traversal components and resolves strictly inside the sticker folder before unlinking. Returns `{ success }` on success or `{ success: false, error }` on failure. | [`app/customStickers/index.js:65`](https://github.com/IsmaelMartinez/teams-for-linux/blob/develop/app/customStickers/index.js#L65) |
| `fetch-giphy-sticker` | Request/Response | Fetch a GIPHY sticker image from the main process (the renderer cannot fetch cross-origin URLs due to Teams' CSP). Returns the image as a base64 data URL so the renderer can build a File for synthetic paste. | [`app/customStickers/index.js:80`](https://github.com/IsmaelMartinez/teams-for-linux/blob/develop/app/customStickers/index.js#L80) |
| `get-sticker-list` | Request/Response | Return the list of stickers (image files) from the configured folder. Each entry includes a base64 data URL so the renderer can show thumbnails and rebuild a File for synthetic-paste insertion without needing direct filesystem access. | [`app/customStickers/index.js:51`](https://github.com/IsmaelMartinez/teams-for-linux/blob/develop/app/customStickers/index.js#L51) |
| `import-sticker-url` | Request/Response | Download an HTTPS URL into the sticker folder. Validates the URL is HTTPS, the response content-type is on the allowlist, and the byte length is under the configured cap. Returns `{ success, filename }` on success or `{ success: false, error }` on failure. | [`app/customStickers/index.js:57`](https://github.com/IsmaelMartinez/teams-for-linux/blob/develop/app/customStickers/index.js#L57) |
| `search-giphy-stickers` | Request/Response | Search GIPHY for stickers. Requires a GIPHY API key configured in customStickers.giphy.apiKey. Returns { success, stickers[] } where each sticker has { url, preview, title, width, height }. | [`app/customStickers/index.js:72`](https://github.com/IsmaelMartinez/teams-for-linux/blob/develop/app/customStickers/index.js#L72) |
